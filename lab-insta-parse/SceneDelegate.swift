...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    private func logOut() {
        ...
        User.logout { [weak self] result in
            ...
            switch result {
            case .success:
                // Make sure UI updates are done on the main thread when initiated from background thread
                DispatchQueue.main.async {
                    // Instantiate the storyboard that contains the view controller that you want to go to (i.e. destination view controller)
                    let storyboard = UIStoryboard(name: Constants.storyboardIdentifier, bundle: nil)
